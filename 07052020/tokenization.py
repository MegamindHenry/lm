...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', action="store", dest="file", type=str, default='trained/sequences_1.txt', help='set save path')
    parser.add_argument('-tp', action="store", dest="tokenizer_path", type=str, default='trained', help='set tokenizer save path')

    args = parser.parse_args()
    
    pathlib.Path(args.tokenizer_path).mkdir(parents=True, exist_ok=True)
    file = args.file
    ...
    print('Saving files...')
    dump(tokenizer, open(os.path.join(tokenizer_path, 'tokenizer.pkl'), 'wb'))
    dump(sequences, open(os.path.join(tokenizer_path, 'sequences.pkl'), 'wb'))